...
_G.Server.Udp.callbacks.recv = function (data, clientid)
    ...
    elseif packet.id == "player_shoot" then
        ...
        if entity then
            ...
            local force = entity:getComponent("Force").force
            local orientation = entity:getComponent("Orientation").orientation
            -- _G.RealmWorld:addEntity(ProjectileEntity:new(uuid(), { x = position.x, y = position.y }, orientation, { width = 8, height = 8 }, 50, force, 400, uid))
        end
